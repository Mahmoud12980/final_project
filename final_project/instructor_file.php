...
if (isset($_POST['save'])) { // if save button on the form is clicked
    ...
    if (empty($vtitle)) {array_push($errors, "File Title Is Required");}
    if (empty($name)) {array_push($errors, "Select File");}

    $user_check_query = "SELECT * FROM files WHERE name='$name' and level='$level' LIMIT 1";
    $result = mysqli_query($conn, $user_check_query);
    $userr = mysqli_fetch_assoc($result);

    if ($userr) { // if video exists
        if ($userr['name'] === $name and $userr['level'] === $level) {
            array_push($errors, "File Already Published");
        }
    }
    ...
    if (count($errors) == 0) {
        ...
        if (!in_array($extension, ['zip', 'pdf', 'docx', 'PNG', 'png', 'jpg', 'JPG', 'mp4', 'MP4'])) {
            array_push($errors, "Extension Unknown");
            ...
            if (move_uploaded_file($file, $destination)) {
                $sql = "INSERT INTO files (dr_name,S_title,level,vtitle,name, size, downloads) VALUES ('$user','$S_title','$level','$vtitle','$name', $size, 0)";
    ...
    <div id="wrapper">
        ...
        <div id="main">
            <div class="inner">
                ...
                <br> <br>
                <div>
                    <form action="instructor_file.php" method="post" enctype="multipart/form-data">
                        <h3>Upload Files</h3>
                        ...
                        <div class="row gtr-uniform">
                            ...
                            <div class="col-6">
                                <input type="text" name="vtitle" id="demo-name" value="" placeholder="File Title" />
                            </div> <br>
                            <div class="col-12">
                            ...
                            <div class="col-12">
                                <input type="file" name="myfile" id="demo-name" value="" />
                            </div> <br>
                            <div class="col-12">
                                <ul class="actions">
                                    <li><input name="save" type="submit" value="Submit" class="primary" /></li>
                                </ul>
                            </div>
                        </div>
                    </form>
